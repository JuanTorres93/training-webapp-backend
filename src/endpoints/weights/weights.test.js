...
describe(`${BASE_ENDPOINT}/{userId}`, () => {
    ...
    describe('put requests', () => {
        ...
        beforeAll(async () => {
            ...
            await request.post(BASE_ENDPOINT + `/${newUser.id}`).send(successfulPutRequest);

            // update weight
            response = await request.put(BASE_ENDPOINT + `/${newUser.id}`).send(successfulPutRequest);
        });
        ...
        describe('unhappy paths', () => {
            describe('400 error when', () => {
                it('weight is not a number', async () => {
                    const response = await request.put(BASE_ENDPOINT + `/${newUser.id}`).send({
                        ...successfulPutRequest,
                        value: 'not a number',
                    });
            ...
            describe('401 error when', () => {
                ...
                it('user is not logged in', async () => {
                    const response = await request.put(BASE_ENDPOINT + `/${newUser.id}`).send(successfulPutRequest);
                    expect(response.statusCode).toStrictEqual(401);
                })
            })
