...
describe(`${BASE_ENDPOINT}` + "/last/{templateId}/user/{userId}", () => {
  ...
  describe("get requests", () => {
    ...
    beforeAll(async () => {
      ...
      const { pushResponse, pushTemplate } = await addWorkoutsAndExercises(
        user.id,
        exercisesIds
      );
      ...
      const workoutReponse = await request.get(
        "/workouts/" + pushResponse.body.id
      );
    ...
    describe("uphappy paths", () => {
      ...
      describe("400 response when", () => {
        it("templateId is string", async () => {
          const response = await request.get(
            BASE_ENDPOINT + "/last/wrongId/user/1"
          );
      ...
      describe("404 response when", () => {
        it("templateId is valid but workout with that id does not exist", async () => {
          ...
          const response = await request.get(
            BASE_ENDPOINT + "/last/" + uuid + "/user/" + uuid
          );
