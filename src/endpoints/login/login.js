...
loginRouter.post("/extend-session", mw.authenticatedUser, (req, res, next) => {
  if (req.session) {
    ...
    req.session.regenerate((err) => {
      if (err) {
        return res
          .status(400)
          .json({ message: "Session could not be extended." });
      } else {
        // Restore the authenticated user in the new session
        req.login(user, (err) => {
          if (err) {
            return res.status(400).json({
              message: "Error restoring user after session regeneration.",
            });
          }
